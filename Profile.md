# MD SAKHAWAT HOSSAIN

📍 Oikotie 1 D21, 68600 Pietarsaari, Finland  
📞 +358 41 729 5788  
📧 Sakatex83@gmail.com  
💬 WhatsApp: +358 41 729 5788  
🔗 GitHub: https://github.com/Sakhawat2  
🔗 LinkedIn: https://www.linkedin.com/in/md-sakhawat-hossain-375192160  

**Gender:** Male  
**Date of Birth:** 11 November 1983  
**Nationality:** Bangladeshi  

---

## WORK EXPERIENCE

### Chef  
**Chef Spartao Oy, Sushibar Pietarsaari (Prisma Pietarsaari)**  
📍 Pietarsaari, Finland  
🗓️ 08/04/2024 – Present  

- Prepare and serve sushi to customers  
- Fish cutting and ingredient preparation  
- Estimate and prepare daily and next day consumption  
- Maintain cleanliness and hygiene standards  
- Follow self-monitoring hygiene plans  

**Achievement:** Best Employee 2024  

---

### Chef  
**Chef Spartao Oy, Sushibar Pietarsaari (Prisma Pietarsaari)**  
📍 Pietarsaari, Finland  
🗓️ 01/05/2017 – 25/12/2023  

- Sushi preparation and customer service  
- Fish cutting and kitchen preparation  
- Inventory estimation and planning  
- Maintain hygienic working environment  

---

### Service Manager  
**Murata Machinery Bangladesh Ltd.**  
*(Sister concern of Murata Machinery Ltd., Japan)*  
📍 Dhaka, Bangladesh  
🗓️ 01/05/2017 – 25/12/2023  

- Troubleshooting, inventory control, and management information systems  
- Machine installation, supervision, and commissioning  
- Electrical, mechanical, and quality troubleshooting  
- Training customers and junior engineers  
- Improve product quality and operational efficiency  
- Managed service schedules for 11 staff serving 150+ customers  
- Served customers in Bangladesh, India, Indonesia, Thailand, and Japan  

**Achievements:**  
- Best Employee (2017–2023)  
- Best Feedback Award 2022  

---

## EDUCATION AND TRAINING

### Bachelor of Engineering (Information Technology)  
**Centria University of Applied Sciences**  
📍 Pietarsaari, Finland  
🗓️ 01/08/2023 – Present  
🌐 https://net.centria.fi  

- EQF Level 6  
- 168 ECTS completed  
- Average Grade: 4.07  

**Key Subjects & Projects:**  
- Python fundamentals, C#, web and cloud development  
- ICT project management and entrepreneurship  
- Data Warehouse project  
- Autonomous car teleoperation (Finland–Germany)  
- Personal Expense Tracker (Web Application)  
- Order System UI project  

---

### Bachelor of Engineering (Electrical and Electronics Engineering)  
**Atish Dipankar University of Science and Technology**  
📍 Dhaka, Bangladesh  
🌐 https://www.adust.edu.bd  

- CGPA: 3.29 / 4.00  

---

## PERSONAL SKILLS

- Communication  
- Teamwork  
- Problem-solving  
- Adaptability  
- Time management  
- Leadership  
- Work ethic  
- Creativity  
- Attention to detail  
- Emotional intelligence  
- Critical thinking  
- Self-motivation  

---

## LANGUAGES

**Mother Tongue:** Bengali  

**Other Languages:**  

| Language | Listening | Reading | Speaking | Writing |
|--------|----------|---------|---------|---------|
| English | Excellent | Excellent | Excellent | Excellent |

---

## COMPUTER SKILLS

- Good command of Microsoft Office tools  

---

## DRIVING LICENCE

- Category B  

---

## PROJECTS

### Payroll Management system
🔗 https://github.com/Sakhawat2/employee-payroll-system


Create a modular and scalable backend system related to employees and payroll. The implementation of a secure and friendly frontend interface dashboard for both employees and administrators. Serve as a basis for automating payroll computations based on submitted work records. Enabling administrative functions like approval processes, editing records, and export of invoice statements. For maintaining data integrity, security, and usability within the system

-----
### -	Smart Parking Management System
🔗 https://github.com/Sakhawat2/Smart-Parking-Management-system


The Smart Parking System project aims to solve urban parking challenges by enabling real-time monitoring, occupancy tracking, and automated gate control. The system integrates IoT hardware, a cloud database, a backend server, and a web dashboard.
The solution uses an Arduino Uno WiFi Rev2, a Grove Ultrasonic Ranger, and Firebase Realtime Database to detect vehicle presence and update the cloud instantly. A web dashboard visualizes this data and allows cloud-to-Arduino commands such as “Gate Open” and “Gate Close.” Urban areas face congestion due to inefficient parking management, leading to wasted time, fuel consumption, and driver frustration. Unlike traditional parking systems, this project integrates real-time sensing with cloud-based analytics and a user-facing dashboard.
Software
 Node.js Express backend,  Firebase Realtime Database, JavaScript Web Dashboard,  Chart.js Visualization Library,  Arduino Firmware (C/C++)

---


### Teleoperation of Autonomous Cars  
**International Software Project (HDA & Centria, Kokkola)**  

- Remote control of model cars across national borders  
- Real-time video streaming and controller-based operation  
- Backend implemented in Python  
- Frontend implemented in C# and Unity  

---

### Data Warehouse Project  
🔗 https://github.com/Sakhawat2/Datawarehouse  

- Centralized data hub for sensor, video, and file storage  
- Structured datasets for analytics and model training  

**Technology Stack:**  
- Backend: FastAPI (Python)  
- Sensor Storage: SQLite  
- Video Storage: Local disk  
- Dashboard: HTML, Chart.js  

---

### Personal Expense Tracker  
🔗 https://github.com/Sakhawat2/Advanced_webdev  

- Personal finance management web application  
- Expense tracking, reporting, and budgeting  

**Technology Stack:**  
- Backend: Node.js, SQLite, JWT Authentication  
- API: REST architecture, Axios  
- Frontend: React (Vite), React Router, Chart.js  

---



